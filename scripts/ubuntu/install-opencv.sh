...
if [ "$arch" = "armv7l" ]; then
	...
	cmake $opencvextramodpath -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=/usr/local -DWITH_QT=OFF -DWITH_FFMPEG=ON -DWITH_GSTREAMER=ON -DWITH_WEBP=ON _DBUILD_opencv_apps=ON .DBUILD_DOCS=ON -DBUILD_EXAMPLES=ON -DBUILD_SHARED_LIBS=ON -DWITH_PNG=ON -DWITH_TIFF=ON -DWITH_XIMEA=ON -DWITH_OPENCL=ON -DWITH_LIBV4L=ON -DWITH_V4L=ON -DWITH_TBB=ON -DBUILD_TBB=ON -DBUILD_EXAMPLES=ON -DBUILD_TESTS=OFF -DBUILD_PERF_TESTS=OFF -DBUILD_JPEG=ON -DENABLE_VFPV3=ON -DENABLE_NEON=ON .. >> $logfile 2>&1
else
	log "Making for X86..."
